...
class MyArticle : Fragment(), OnItemClickListener {

    var arrayList = ArrayList<myartcl>()
    ...
    override fun onItemClicked(myartcl: myartcl, sec: Int) {
        if (sec == 1) {
            val intent = Intent(requireContext(), ActivityAddArticle::class.java)
            intent.putExtra("ID", myartcl.id.toString())
            startActivity(intent)
        } else {
            val builder = AlertDialog.Builder(requireContext())
            builder.apply {
                setPositiveButton("Ok" ) { dialog, id ->
                    delete(myartcl.id.toString())
                }
                ...
                override fun onResponse(response: JSONObject?) {
                    if (response?.getString("result").toString() == "Data kosong") {
                        ...
                        for (i in 0 until jsonArray?.length()!!) {
                            ...
                            arrayList.add(myartcl(jsonObject.getInt("id"),
                                    jsonObject.getString("judul"),
                                    jsonObject.getString("picture")))

                            if (jsonArray.length() - 1 == i) {
                                loading.dismiss()
                                val adapter = AdapterMyArticle(this@MyArticle, arrayList)
